...
namespace HREngine.Bots
{
    ...
    class Sim_EDR_454 : SimTemplate
    {
        ...
        {
            if (target != null && target.own && RaceUtils.MinionBelongsToRace(target.handcard.card.GetRaces(),CardDB.Race.DRAGON))
            {

                Minion summoned = p.callKidAndReturn(kid, triggerMinion.zonepos, triggerMinion.own);
                if (summoned != null)
                    summoned.deathrattle2 = target.handcard.card;
